...
public sealed class ErrorTaggerProvider: IViewTaggerProvider {
    ...
    public ITagger<T> CreateTagger<T>(ITextView textView, ITextBuffer buffer) where T : ITag {
        ...
        if (_createdTaggers.TryGetValue(textView, out ErrorTagger createdTagger)) {
            return createdTagger as ITagger<T>;
        }
    ...
    public void Rerender() {
        ...
        foreach (KeyValuePair<ITextView, ErrorTagger> createdTagger in _createdTaggers) {
            createdTagger.Value.Rerender();
        }
    }
}